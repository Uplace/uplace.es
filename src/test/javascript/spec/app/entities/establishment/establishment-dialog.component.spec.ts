/* tslint:disable max-line-length */
import { ComponentFixture, TestBed, async, inject, fakeAsync, tick } from '@angular/core/testing';
import { NgbActiveModal } from '@ng-bootstrap/ng-bootstrap';
import { Observable } from 'rxjs/Observable';
import { JhiEventManager } from 'ng-jhipster';

import { UplaceTestModule } from '../../../test.module';
import { EstablishmentDialogComponent } from '../../../../../../main/webapp/app/entities/establishment/establishment-dialog.component';
import { EstablishmentService } from '../../../../../../main/webapp/app/entities/establishment/establishment.service';
import { Establishment } from '../../../../../../main/webapp/app/entities/establishment/establishment.model';

describe('Component Tests', () => {

    describe('Establishment Management Dialog Component', () => {
        let comp: EstablishmentDialogComponent;
        let fixture: ComponentFixture<EstablishmentDialogComponent>;
        let service: EstablishmentService;
        let mockEventManager: any;
        let mockActiveModal: any;

        beforeEach(async(() => {
            TestBed.configureTestingModule({
                imports: [UplaceTestModule],
                declarations: [EstablishmentDialogComponent],
                providers: [
                    EstablishmentService
                ]
            })
            .overrideTemplate(EstablishmentDialogComponent, '')
            .compileComponents();
        }));

        beforeEach(() => {
            fixture = TestBed.createComponent(EstablishmentDialogComponent);
            comp = fixture.componentInstance;
            service = fixture.debugElement.injector.get(EstablishmentService);
            mockEventManager = fixture.debugElement.injector.get(JhiEventManager);
            mockActiveModal = fixture.debugElement.injector.get(NgbActiveModal);
        });

        describe('save', () => {
            it('Should call update service on save for existing entity',
                inject([],
                    fakeAsync(() => {
                        // GIVEN
                        const entity = new Establishment(123);
                        spyOn(service, 'update').and.returnValue(Observable.of(entity));
                        comp.establishment = entity;
                        // WHEN
                        comp.save();
                        tick(); // simulate async

                        // THEN
                        expect(service.update).toHaveBeenCalledWith(entity);
                        expect(comp.isSaving).toEqual(false);
                        expect(mockEventManager.broadcastSpy).toHaveBeenCalledWith({ name: 'establishmentListModification', content: 'OK'});
                        expect(mockActiveModal.dismissSpy).toHaveBeenCalled();
                    })
                )
            );

            it('Should call create service on save for new entity',
                inject([],
                    fakeAsync(() => {
                        // GIVEN
                        const entity = new Establishment();
                        spyOn(service, 'create').and.returnValue(Observable.of(entity));
                        comp.establishment = entity;
                        // WHEN
                        comp.save();
                        tick(); // simulate async

                        // THEN
                        expect(service.create).toHaveBeenCalledWith(entity);
                        expect(comp.isSaving).toEqual(false);
                        expect(mockEventManager.broadcastSpy).toHaveBeenCalledWith({ name: 'establishmentListModification', content: 'OK'});
                        expect(mockActiveModal.dismissSpy).toHaveBeenCalled();
                    })
                )
            );
        });
    });

});
